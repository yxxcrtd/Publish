...
<tr class='tdbg' onmouseout="this.className='tdbg'" onmouseover="this.className='tdbgmouseover'">
 ...
 <td width='54' align='center'>
  #{switch channel.status }
   #{case 0}正常
   #{case 1}<font color='red'>停用</font>
   #{case 2}<font color='red'>已删除</font>
  #{/switch }
 </td>
 <td width='110' align='center'>
  <a href='admin_channel_add.jsp?channelId=#{channel.id}'>修改</a>
  #{if(channel.status == 0) }
   <a href='admin_channel_action.jsp?command=disable&channelId=#{channel.id}'
     onclick='return confirmDisable();'>禁用</a>
  #{else }
   <a href='admin_channel_action.jsp?command=enable&channelId=#{channel.id}'
     onclick='return confirmEnable();'>启用</a>
  #{/if}
  #{if(channel.channelType != 0) }
   <a href='javascript:deleteChannel(#{channel.id}, "#{channel.name}")'>删除</a>
  #{else }
...
<form name="myform" method="post" action="admin_channel_action">
<div align='center' style="display:none">
 <input type='hidden' name='action' id='Action' value='UpdateData' />
 <input type='submit' name='submit' value='更新所有频道的数据' onclick="byId('Action').value='updateData'" style="display:none"/>
 <input type='submit' name='submit' value='更新所有频道的文件' onclick="byId('Action').value='updateFile'" />
</div>
</form>
</pub:template>
</pub:declare>
