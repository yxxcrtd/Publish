...
public class DataSchema extends java.util.ArrayList<String> {
	...
	public static final String[] parseSelectFields(String fields) {
		...
		String[] cols = StringHelper.split(fields, ",", false);
	...
	private static final String parseSingleField(String field) {
		...
		field = field.trim();
		// 格式可能为 'title', 'alias.title', 'title as articleTitle', 'alias.title as articleTitle'
		// 我们假定用户写的比较规则，不要使用特殊的东西。
		String[] tokens = field.split("\\s");	// 分解为基本标记。
		if (tokens.length == 0) return "";
		if (tokens.length == 1) return parseFieldFinal(tokens[0]);
		if (tokens.length == 2) return parseFieldFinal(tokens[1]);	// 'title Title'
		// else > 2 field, using last as alias name
