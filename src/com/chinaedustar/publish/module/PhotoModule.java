...
/**
 * 图片管理模块的实现类。此模块用于支持图片等图片显示频道。
 * 
 * @author liujunxing
 * @ModuleName - PhotoModule
 */
public class PhotoModule extends AbstractItemModule {
	/**
	 * 构造。
	 *
	 */
	...
	public Menu getChannelMenu(Channel channel, Admin admin) {
		...
		Menu menu = new Menu("photo_" + channel.getChannelDir() + "_" + channel.getId(), 
				channel.getName());
		menu.setUrl("admin_channel_guide.jsp?channelId=" + channel.getId());
		builder.initSetMenu(menu);
		
		builder.addItemMenu("photo");		// '添加图片', '我添加的图片'
		builder.addPhotoItemManageMenu();	// '图片管理', '审核', '生成'
		builder.addSpecialItemMenu("photo");// '专题图片管理'
		builder.addCommentMenu();			// '评论管理'
		builder.addRecycleMenu("photo");	// '图片回收站管理'
		
		builder.addSeperator();			// =======================
		builder.addChannelSetting();	// '图片中心设置'
	 ...
	 */
	public Photo loadItem(int itemId) {
