...
public class OptimizerUtil {
	private static final BigInteger INTEGER_MIN = new BigInteger(
			Integer.toString(Integer.MIN_VALUE));
	private static final BigInteger INTEGER_MAX = new BigInteger(
			Integer.toString(Integer.MAX_VALUE));
	private static final BigInteger LONG_MIN = new BigInteger(
			Long.toString(Long.MIN_VALUE));
	private static final BigInteger LONG_MAX = new BigInteger(
	...
	public static List optimizeListStorage(List list) {
		...
		case 0: {
			...
			/*
			 * case 1: { return Collections12.singletonList(list.get(0)); }
			 */
		default: {
	 ...
	 * original number type, we have no other choice to ensure expected
	 * operation in majority of cases.
	 */
	public static Number optimizeNumberRepresentation(Number number) {
		...
		if (number instanceof BigInteger) {
			...
			if (bi.compareTo(INTEGER_MAX) <= 0
					&& bi.compareTo(INTEGER_MIN) >= 0) {
				// BigInteger -> Integer
